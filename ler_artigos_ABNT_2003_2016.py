...
def transformar_texto(texto:str) -> str:
    ...
    texto = texto.replace(" ¸", "¸").replace("` ", "`").replace("´ ", "´").replace("ˆ ", "ˆ").replace("˜ ", "˜")
    texto = texto.replace("¸c", "ç").replace("c¸", "ç").replace("`a", "à").replace("`A", "À")
...
def ler_transformar_pdf_ABNT(link_pdf:str, ano_evento:int) -> list:
    anais = []
    inicio_artigos = 0
    ...
    for page_num in range(len(pdf_document)):
        ...
        if (text.find("Abstract.") != -1 or text.find("Abstract -") != -1):
                ...
                if "lines" in block:
                    for line in block["lines"]:
                        for span in line["spans"]: 
                            ...
                            if texto_base.strip() == "" or texto_base == '\u0001':
                                #print("Pulou")
                                continue
                            ...
                            #print(f"{fl_ordem} - {fl_universidade}")
                            
                            # Titulo: Fonte >= 14.5 e texto com mais de 2 caracteres
                            if font_size >= 14.5 and len(texto_base) > 2:
                            ...
                            #Regras para identificar os dados de autores, universidade e a ordem, que são os mais difíceis de identificar, pois não possuem uma formatação tão clara quanto os dados de título e autores. A ideia é usar uma combinação de regras para tentar identificar esses dados da melhor forma possível, mesmo que haja algumas inconsistências na formatação dos PDFs.
                                ...
                                if font_name in('CMMI8', 'CMSY8') and len(texto_base) <= 3:
                                    ordem += texto_base
                                    fl_ordem = False
                                    
                                # Ordem: Fonte de tamanho <= 8.1, sem fl_ordem e sem fl_universidade
                                ...
                                elif (font_name == 'NimbusRomanNo9L' or font_name == 'CMBX12' or font_name.find("Medi") != -1 or font_name.find("Bold") != -1) and not fl_universidade and font_size >= 10.5:
                                ...
                                elif (font_name == 'NimbusRomanNo9L' or font_name == 'CMR12' or font_name.find("Regu") != -1 or font_name.find("Times") != -1) and font_size > 8.1:
            ...
            if (text.find("Resumo.") != -1 or text.find("Resumo -") != -1):
                ...
                resumo = resumo.split("Abstract", 1)[0].strip() if resumo.find("Abstract.") != -1 or resumo.find("Abstract -") != -1 else resumo
            else:
                lingua = 'En'
